...
function mvt_register_user() {
    ...
    if(isset($_POST['submitted'])) {
        ...
        if(is_wp_error($reg_errors)) {
            foreach($reg_errors->get_error_messages() as $error) {
                ?>
                    <div style="color:#FF0000; text-align: left;"><?= $error ?></div>
                <?php
            }
        }
    ...
    ?>
        ...
        <form action="" method="POST" name="user_registeration">
            ...
            <input type="submit" name="user_registeration" value="<?= esc_attr__('Sign Up', 'mv-translations') ?>">

            <input type="hidden" name="mv_translations_register_nonce" value="<?= wp_create_nonce('mv_translations_register_nonce'); ?>">
