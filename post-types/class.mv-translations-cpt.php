...
if(! class_exists('MV_Translations_Post_Type')) {
    class MV_Translations_Post_Type {
        ...
        public function add_meta_boxes() {
            add_meta_box('mv_translations_meta_box', esc_html__('Translations Options', 'mv-translations'), [$this, 'add_inner_meta_boxes'], 'mv-translations', 'normal', 'high');
        }
        ...
        public static function save_post($post_id, $post) {
            ...
            if(defined('DONIG_AUTOSAVE') && DOING_AUTOSAVE) {
                return;
            }
            ...
            if(isset($_POST['action']) && $_POST['action'] == 'editpost') {
                ...
                if($_POST['mv_translations_action'] == 'save') {
                    if(get_post_type($post) == 'mv-translations' &&
                        ...
                        $post->post_status != 'darf' &&
                        $wpdb->get_var(
                            $wpdb->prepare(
                                "SELECT translation_id FROM $wpdb->translationmeta WHERE translation_id = %d",
                                $post_id
                            )) == null
                    ) {
                        $wpdb->insert(
