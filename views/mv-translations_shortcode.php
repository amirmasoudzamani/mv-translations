...
<div class="mv-translations">
    <form action="" method="POST" id="translations-form">
        ...
        <?php
        if(isset($content)) {
            wp_editor($content, 'mv_translations_content', ['wpautop' => true, 'media_buttons' => false]);
        } else {
            wp_editor('', 'mv_translations_content', ['wpautop' => true, 'media_buttons' => false]);
        }
        ?>
        <br>
        ...
        <input type="submit" name="submit-form" value="<?php esc_attr_e('Submit', 'mv-translations') ?>">
    </form>
</div>
        ...
        <tbody>
            <?php foreach($results as $result): ?>
                <tr>
                    ...
                    <td><?= $result->meta_value == 'Yes' ? esc_html__('Yes', 'mv-translations') : esc_html__('No', 'mv-translations') ?></td>
                    <?php $edit_post = add_query_arg('post', $result->ID, home_url('/edit-translation')) ?>
                    <td><a href="<?= esc_url($edit_post) ?>"><?php esc_html_e('Edit', 'mv-translations') ?></a></td>
                    <td><a onclick="return confirm('Are you sure you want to delete post: <?= $result->post_title ?>?')" href="<?= get_delete_post_link($result->ID, "", true) ?>"><?php esc_html_e('Delete', 'mv-translations') ?></a></td>
                    <td><?= $result->post_status == 'Publish' ? esc_html__('Published', 'mv-translations') : esc_html__('Pending', 'mv-translations') ?></td>
                </tr>
            <?php endforeach ?>
        </tbody>
    </table>
